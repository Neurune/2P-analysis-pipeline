function y = prctileFilt(x,n,blksz,DIM)
xStart = n+1;
xStop = length(x)-n;
xLength = length(x);
x = [fliplr(x(1:xStart)),x,fliplr(x(xStop:end))];
...
for i=1:blksz:nx
    ...
    y(i:min(i+blksz-1,nx)) = prctile(xx,30); %Play around with this value
end
end
y = y(xStart:xStart+xLength-1);
end